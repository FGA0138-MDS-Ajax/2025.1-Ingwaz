...
export default function WeatherScreen() {
  ...
  const fetchWeatherList = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.daily && data.daily.time) {
        // Transformando os dados para um formato mais fácil para a FlatList
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {carregandoLista ? (
        <ActivityIndicator size="large" color="#2c6e49" style={{ marginTop: 20 }} />
      ) : erro ? (
        ...
        <>
          {selectedDay && (
            <View style={styles.detailContainer}>
              ...
              {carregandoDetalhe ? (
                ...
                <>
                  <Text>Temperatura Máx: {selectedDay.temperature_2m_max}{selectedDay.units?.temperature_2m_max}</Text>
                  <Text>Temperatura Mín: {selectedDay.temperature_2m_min}{selectedDay.units?.temperature_2m_min}</Text>
                  <Text>Precipitação: {selectedDay.precipitation_sum}{selectedDay.units?.precipitation_sum}</Text>
                  <Text>Vento: {selectedDay.windspeed_10m_max}{selectedDay.units?.windspeed_10m_max}</Text>
                </>
              )}
            </View>
          )}
          ...
          <FlatList
            ...
            ListEmptyComponent={
              !carregandoLista && <Text style={styles.emptyMessage}>Nenhuma previsão encontrada.</Text>
            }
          />
        </>
      )}
